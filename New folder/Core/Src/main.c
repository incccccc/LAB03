		 ...
		 if (MotorSetRPM>=4)
		 {
			 duty = RPMset*10000;
					 if (HAL_GetTick()>= timestamp);
								 ...
								 MotorReadRPM = 60/(IC_Calc_Period() * 0.000001*768);
								 RPMset = MotorSetRPM/9.5;
		 }
		 else if (MotorSetRPM<4 && MotorSetRPM>=3 )
		 		 {
		 			 duty = RPMset*10000;
		 					 if (HAL_GetTick()>= timestamp);
		 								 {
		 								 ...
		 								 MotorReadRPM = 60/(IC_Calc_Period() * 0.000001*768);
		 								 RPMset = MotorSetRPM/8;
		 		 }
		 else if (MotorSetRPM<3 && MotorSetRPM>=2 )
		 		 		 {
		 		 			 duty = RPMset*10000;
		 		 					 if (HAL_GetTick()>= timestamp);
		 		 								 {
		 		 								 ...
		 		 								 MotorReadRPM = 60/(IC_Calc_Period() * 0.000001*768);
		 		 								 RPMset = MotorSetRPM/6.5;
		 		 		 }
		 else if (MotorSetRPM<=2)
				 		 {
				 			 duty = RPMset*10000;
				 					 if (HAL_GetTick()>= timestamp);
				 								 {
